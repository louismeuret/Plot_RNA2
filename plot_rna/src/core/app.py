...
# Import celery app
try:
    from tasks_celery import app as app2
except ImportError:
    # Fallback to creating a simple AsyncResult handler
    app2 = None
import threading
import logging
...
def retrieve_results():
    ...
    explanations_path = os.path.join(static_dir, 'explanations.json')
    if not os.path.exists(explanations_path):
        explanations_path = os.path.join(parent_dir, 'static', 'explanations.json')
...
def view_trajectory(session_id, native_pdb, traj_xtc):
        ...
        for plot in submittable_plots:
            try:
                ...
                if job_info:
                    ...
                    active_jobs[plot] = job_info[2]  # Store the job object
            ...
            if job.ready():
                ...
                try:
                    if job.successful():
                        ...
                        for plot_data_entry in plot_data:
                            if plot_data_entry[0] == plot and plot_data_entry[2] == job:
                                plot_data_entry[2] = job.result  # Replace job object with result
                                logger.info(f"Collected result for {plot}")
                                
                                # Update progress
                                completed_count = len(completed_dependencies)
                                total_count = len(optimized_order)
                                progress = 60 + (completed_count / total_count) * 20
                                socketio.emit('update_progress', {"progress": progress, "message": f"{plot} completed ({completed_count}/{total_count})"}, to=session_id)
                                socketio.sleep(0.1)
                                break
                    else:
                        logger.error(f"Job failed for {plot}: {job.result}")
                        socketio.emit('update_progress', {"progress": 80, "message": f"Error with {plot}: {job.result}"}, to=session_id)
                except Exception as e:
                    logger.error(f"Error collecting result for {plot}: {str(e)}")
                    socketio.emit('update_progress', {"progress": 80, "message": f"Exception with {plot}: {str(e)}"}, to=session_id)
    ...
    socketio.emit('update_progress', {"progress": 90, "message": "All jobs completed, processing results..."}, to=session_id)
    socketio.sleep(0.1)  # Ensure the message is sent

    # Process collected results
    completed_plot_data = []
    for plot_type, plot_style, result_data in plot_data:
        try:
            # result_data is now the actual result, not a job object
            if result_data is not None:
                completed_plot_data.append([plot_type, plot_style, result_data])
                logger.info(f"Added {plot_type} result to completed data")
            else:
                logger.warning(f"No result data for {plot_type}")
        except Exception as e:
    ...
    return render_template(
            ...
            trajectory_length=len(u.trajectory),
            explainations=explanations,
            rotation_matrix=str(r_matrix.tolist()) if r_matrix is not None else "[]",
        )
